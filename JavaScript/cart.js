...
class CartManager {
            ...
            <div class="checkout-overlay">
                <div class="checkout-content">
                    <div class="checkout-logo">
                        <!-- CORRECCIÓN: Ruta dinámica para que funcione en index.html y en carpetas -->
                        <img src="${logoPath}" alt="UrbanHustler" onerror="this.onerror=null; this.src='https://via.placeholder.com/150/000000/FFFFFF/?text=UH'">
                    </div>
    ...
    handleCheckout(e) {
        ...
        if (!userToken) {
            ...
            if (loginContainer) {
                ...
                loginContainer.style.outline = "2px solid #8C8C8C";
                loginContainer.style.outlineOffset = "4px";
                setTimeout(() => {
                    loginContainer.style.outline = "";
                    loginContainer.style.outlineOffset = "";
                }, 700);
            }
            return;
        }
        ...
        setTimeout(() => {
            ...
            this.cart.forEach(item => {
                message += `▪️ [COD: ${item.id}] ${item.name} (Talle: ${item.size}) x${item.quantity}\n`;
            });

            const total = window.formatPrice ? window.formatPrice(this.calculateTotal()).replace('$', '') : this.calculateTotal().toLocaleString();
            ...
            // FIX: Estructura <a> para compatibilidad con Safari (iPhone)
            const link = document.createElement('a');
            link.href = url;
            
            // Safari bloquea popups en timeouts. En iOS usamos _self para forzar la app.
            ...
            // Se comenta el vaciado automático para que el usuario no pierda el carrito si regresa
            // this.cart = [];
            // this.saveCart();
            this.renderCart();
            this.closeCart();
    ...
    recordOrder() {
        ...
        const newOrder = {
            ...
            status: 'pending' // Asegurar que tenga estado para aparecer en el panel
        };
        ...
        if (window.firebaseManager) {
            window.firebaseManager.saveOrder(newOrder);
        }
    }
    ...
    addItem(product, quantity = 1, size = 'M') {
        // 🔒 SEGURIDAD: Verificar Login antes de permitir agregar
        const userToken = localStorage.getItem('user_token');
        if (!userToken) {
            this.showNotification("🔒 Inicia sesión para comprar");
            const loginBtn = document.getElementById('google-login-container');
            if (loginBtn) {
                loginBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
                loginBtn.style.transition = "transform 0.3s";
                loginBtn.style.transform = "scale(1.2)";
                setTimeout(() => loginBtn.style.transform = "scale(1)", 500);
            }
            return;
        }
    ...
    renderCart() {
        ...
        this.cart.forEach((item, index) => {
            ...
            const optimizedImg = window.optimizeCloudinary(item.image, 150);

            cartItem.innerHTML = `
                <img src="${optimizedImg}" alt="${item.name}" 
                     width="90" height="110" loading="lazy">
        ...
        if (this.cartTotalElement) {
            this.cartTotalElement.textContent = window.formatPrice ? window.formatPrice(this.calculateTotal()) : `$${this.calculateTotal().toLocaleString()}`;
        }
        this.updateCartCount();
    }
